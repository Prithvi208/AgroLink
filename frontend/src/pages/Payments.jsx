            ...
            <div key={order.id} className="bg-yellow-50 border border-yellow-200 rounded-2xl p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Amount: <span className="font-bold text-agro-700">{formatCurrency(order.total_price)}</span></p>
                </div>
          ...
          <div key={p.id} className="bg-white rounded-2xl shadow-sm border p-5 flex items-center gap-4">
            ...
            <div style={{textAlign: 'right'}}>
              <p style={{fontWeight: 'bold', color: p.payer_id === user?.id ? '#dc2626' : '#16a34a'}}>
                {p.payer_id === user?.id ? '-' : '+'}{formatCurrency(p.amount)}
              </p>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            ...
            <p className="text-3xl font-bold text-red-600 mt-1">{formatCurrency(totalPaid)}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            <p className="text-sm text-gray-500">Total Received</p>
            <p className="text-3xl font-bold text-green-600 mt-1">{formatCurrency(totalReceived)}</p>
          </div>
        </div>
